...
define(["jquery", "underscore", "backbone", "util/appDirCommon", "workers/dataPoster", "model/gitHubFileCollection",
    "model/importForm", "view/sideBar", "view/ghViewDataModal", "view/progressBar", "hb!template/viewFileModal.part.hbs",
    "hb!template/importForm.hbs", "util/uiUtils"],
    function ($, _, Backbone, cu, dataPoster, GitHubFileCollection, ImportFormModel, SideBarView, GHViewDataModal, ProgressBarView,
              viewFileModal, compiledImportFormTmpl, uiUtils) {
...
//                var url = that.postParams.appdhost + "/darwin/#false:applicationOverviewPage:" + data.applicationId;
                    ...
                    $.when(uiUtils.openWindow(base64URL)).
                        done(function () {
                            cu.log("openWindow resolved successfully");
                        }).
                        fail(function () {
                            var context = {
                                headerText:"Application Director Open View Failed",
                                bodyText:'<b><p style="font-size: 15px" class="text-info">' +
                                    'There was a problem opening a window with the successfully imported blueprint. ' +
                                    'This is more than likely a result of your browser\'s popup blocker, if this is the case ' +
                                    'you can set the browser to allow popups from this site in the future. <br/>' +
                                    'Here is the link that was attempted: ' + base64URL +
                                    '</p></b>'
                            };
        ...
        ImportExportApp.prototype.bindImportForm = function () {
            ...
            $("#importButton").bind("click", _.bind(function () {
                var uname = $("#appDirUserName").val() ? $("#appDirUserName").val() : $("#appDirUserName").attr("placeholder");
                var password = $("#appDirPassword").val() ? $("#appDirPassword").val() : $("#appDirPassword").attr("placeholder");
                var appdhost = $("#appDirHost").val() ? $("#appDirHost").val() : $("#appDirHost").attr("placeholder");
                var bytes =
                ...
                var importAsNewSuffix =
                    (conflictResolution == "IMPORTASNEW") ?
                        ($("#importAsNewSuffix").val() ? $("#importAsNewSuffix").val() : $("#importAsNewSuffix").attr("placeholder")) : null;
        ...
        if (!$(document.documentElement) || (!$(document.documentElement).hasClass("chrome-gte23") && !$(document.documentElement).hasClass("ff-gte16"))) {
            var context = {
                headerText:"Unsupported Browser",
                bodyText:'<b><p style="font-size: 15px" class="text-info">The latest version of Chrome or Firefox is required to work with this page.</p></b> ' +
                    '<a href="http://www.google.com/chrome"><img src="../images/chrome_logo_2x.png" class="img-rounded"></a> <br/> ' +
                    '<a href="http://www.mozilla.org/en-US/firefox/"><img src="../images/ff-usage-standard.png" class="img-rounded"></a>'
            };
            uiUtils.noticeModal(context);
        } else {
